...
#if Mono
                if (_keysToCheck == null) _keysToCheck = Enum.GetValues(typeof(KeyCode)).Cast<KeyCode>().Except(new[] { KeyCode.Mouse0, KeyCode.None });
                foreach (var key in _keysToCheck)
                {
                    if (InputSystemHelper.GetKeyUp(key))
                    {
                        setting.Set(key);
        ...
        private static void DrawKeyboardShortcut(SettingEntryBase setting)
        {
            if (ReferenceEquals(_currentKeyboardShortcutToSet, setting))
            {
                ...
                foreach (var key in _keysToCheck)
                {
                    if (InputSystemHelper.GetKeyUp(key))
                    {
                        setting.Set(new KeyboardShortcut(key, _keysToCheck.Where(InputSystemHelper.GetKey).ToArray()));
                        _currentKeyboardShortcutToSet = null;
                        break;
                    }
                }
