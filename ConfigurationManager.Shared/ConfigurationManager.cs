...
namespace ConfigurationManager
{
    ...
    public class ConfigurationManager : BaseUnityPlugin
    {
        ...
        private void Awake()
        {
            var go = new GameObject(nameof(ConfigurationManagerMonoBehaviour));
            DontDestroyOnLoad(go);
            go.hideFlags = HideFlags.HideAndDontSave;
            go.AddComponent<ConfigurationManagerMonoBehaviour>().Init(this);

            InputSystemHelper.RegisterHotkeyChecker(() =>
            {
                if (OverrideHotkey)
                    return;
...
#if Mono
                Vector2 mousePosition = InputSystemHelper.GetMousePosition();
#else
...
#if !Mono && !IL2CPP
        private void OnGUI() => PluginOnGUI();
#endif
...
#if !Mono && !IL2CPP
        private void Update() => PluginUpdate();
#endif

        internal void PluginLateUpdate()
        {
            if (DisplayingWindow) SetUnlockCursor(0, true);
        }
#if !Mono && !IL2CPP
        private void LateUpdate() => PluginLateUpdate();
#endif
#if !Mono && !IL2CPP
        private void Start() => PluginStart();
#endif
